...
lags = [1] # only use 1 lag for the random walk model

# output directory
odir = wdir + "S04_FAR_random_walk/steps" + str(n_steps_ahead) + "ahead/" 
...
# %%
# to ensure fair comparision (i.e., using same set of test days), keep only dates that are also used in fNTK
fNTK_res = pd.read_csv(wdir + "S02_FAR_NTK/" + obsplines_type + "/steps" + str(n_steps_ahead) + "ahead/HiddenLayers3/" + option_type + "_pred_test_IV.csv")

all_test_preds = all_test_preds[all_test_preds["test_day_ahead_date"].isin(fNTK_res["test_day_ahead_date"].values)]
