...
# output directory
odir = wdir + "S02_FAR_KRR/" + kernel_type + "/steps" + str(n_steps_ahead) + "ahead/" 
...
gamma_vals = [0.05] # [0.02, 0.03, 0.05]
ridge_alpha_vals = [0.5] # [1e-1, 1e-3, 1e-5]
selected_hyperparams = tuning_KRR(train_x = X[0:(n_trainval-n_val),:], 
...
train_dates = split_dates.iloc[0:n_trainval]
test_date = split_dates.iloc[n_trainval:n_days]

# train set
train_x = X[0:n_trainval,:]
train_y = Y[0:n_trainval,:]

# test set
test_x = X[n_trainval:n_days,:]
test_y = Y[n_trainval:n_days,:]
