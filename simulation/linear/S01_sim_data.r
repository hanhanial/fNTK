...
# wdir = "~/Desktop/Hannah/work/SPX/Analysis/simulation_AHBS/all_params_linear/"
...
num_sims = 100
noise_var = 0.0001
num_VAR_processes = 1 # number of VAR(p) processes to be simulated
num_lags = 1 # number of lags
...
for (i in 1:num_sims) {
  ...
  for (k in 1:length(coef_names)) {
    ...
    unconditional_var = params_stats$var[params_stats$coef==coef]
    unconditional_mean = params_stats$mean[params_stats$coef==coef]
    coef_innovation_var = unconditional_var/50
    
    sim_coef = simulateVAR(num_processes = num_VAR_processes,
                           p = num_lags,
                           init = matrix(runif(1),nrow = num_lags,ncol = num_VAR_processes),
                           pars = list(as.matrix(sqrt((unconditional_var - coef_innovation_var)/unconditional_var))),
                           means = rep(unconditional_mean,num_VAR_processes),
                           inno_var = diag(coef_innovation_var,num_VAR_processes),
                           nT = num_points,
  ...
  # add noise to IV
  out2$IV = out2$IV + rnorm(1,0, noise_var)
