...
options = options %>%
  filter(PC_flag == 1, # keep only call options
...
###### Short Call DH ######
short_call = read_csv(paste0("S03_trading_returns/ShortCallDH_overall_thrs",
                             as.character(filtering_thrsh),"_EffSpread",
                             as.character(effective_spread_measure),".csv"))

unique(short_call$model)
...
###### Long Call DH ######
long_call = read_csv(paste0("S03_trading_returns/LongCallDH_overall_thrs",
                            as.character(filtering_thrsh),"_EffSpread",
                            as.character(effective_spread_measure),".csv"))
long_call = long_call %>% 
  mutate(model = factor(x = model,
