...
if (compute_breakdown) {
  short_call_DH_by_M = list(); short_call_DH_by_tau = list()
}
...
for (i in 1:length(all_steps_ahead)) {
  ...
  # since for short call DH, we sell options and buy underlying stock on day t and 
  # buy options and sell underlying stock on day t+h
  # we compute the sell option price (SellPremium_t) and the buy stock price (BuySpot_t) on day t 
  # and buy price (BuyPremium_th) and the sell stock price (SellSpot_th) on day t+h
  ...
  dat = dat %>% 
    mutate(SellPremium_t = MidPrice_t - Spread_t*effective_spread_measure/2,
           ...
           BuySpot_t = Spot_close_t, #+ SpotSpread_t*effective_spread_measure/2,
           SellSpot_th = Spot_close_th #- SpotSpread_th*effective_spread_measure/2
    )
  
  ########## DH returns overall ########## 
  out = short_call_DH(data = dat, 
  ...
  if (compute_breakdown) {
    ...
    for (i in 1:length(M_groups)) {
      ...
      short_call_DH_out = short_call_DH(data = dat %>% 
                                           filter(M_group == M_groups[i]),
                                         steps_ahead = steps_ahead)
      short_call_DH_by_M[[paste0(M_groups[i],"_steps",steps_ahead,"ahead")]] = short_call_DH_out$day_by_day_res %>% 
    ...
    for (i in 1:length(tau_groups)) {
      ...
      short_call_DH_out = short_call_DH(data = dat %>% 
                                           ...
                                           steps_ahead = steps_ahead)
      short_call_DH_by_tau[[paste0(tau_groups[i],"_steps",steps_ahead,"ahead")]] = short_call_DH_out$day_by_day_res %>% 
...
write_csv(overall_day_by_day_res,paste0("S03_trading_returns/ShortCallDH_overall_thrs",
...
if (compute_breakdown) {
  short_call_DH_by_M = Reduce(x = short_call_DH_by_M,f = bind_rows)
  write_csv(short_call_DH_by_M,paste0("S03_trading_returns/ShortCallDH_by_M_thrs",
                                      as.character(filtering_thrsh),"_EffSpread",
                                      as.character(effective_spread_measure),".csv"))
  
  short_call_DH_by_tau = Reduce(x = short_call_DH_by_tau,f = bind_rows)
  write_csv(short_call_DH_by_tau,paste0("S03_trading_returns/ShortCallDH_by_tau_thrs",
                                        as.character(filtering_thrsh),"_EffSpread",
                                        as.character(effective_spread_measure),".csv"))
}
