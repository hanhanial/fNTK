...
if (compute_breakdown) {
  long_put_DH_by_M = list(); long_put_DH_by_tau = list()
}
...
for (i in 1:length(all_steps_ahead)) {
  ...
  if (compute_breakdown) {
    ...
    for (i in 1:length(M_groups)) {
      ...
      long_put_DH_out = long_put_DH(data = dat %>% 
    ...
    for (i in 1:length(tau_groups)) {
      ...
      long_put_DH_out = long_put_DH(data = dat %>% 
                                        ...
                                        steps_ahead = steps_ahead)
      long_put_DH_by_tau[[paste0(tau_groups[i],"_steps",steps_ahead,"ahead")]] = long_put_DH_out$day_by_day_res %>% 
...
write_csv(overall_day_by_day_res,paste0("S03_trading_returns/LongPutDH_overall_thrs",
...
if (compute_breakdown) {
  ...
  write_csv(long_put_DH_by_M,paste0("S03_trading_returns/LongPutDH_by_M_thrs",
                                      as.character(filtering_thrsh),"_EffSpread",
                                      as.character(effective_spread_measure),".csv"))
  
  long_put_DH_by_tau = Reduce(x = long_put_DH_by_tau,f = bind_rows)
  write_csv(long_put_DH_by_tau,paste0("S03_trading_returns/LongPutDH_by_tau_thrs",
                                        as.character(filtering_thrsh),"_EffSpread",
                                        as.character(effective_spread_measure),".csv"))
}
